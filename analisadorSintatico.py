...
class Parser(object):
    ...
    def variableDeclarators(self):
        ...
        if (self.variableDeclarator()):
            ...
            while self.tokenAtual.valor == ",":
                self.proximoToken()
                if not self.variableDeclarator():
                    return False
                self.proximoToken()
            return True
        else:
    ...
    def qualifiedIdentifier(self):
        ...
        if self.analisaIdentificador():
            ...
            while self.tokenAtual.valor == ".":
                self.proximoToken()
                if not self.analisaIdentificador():
                    return False
                self.proximoToken()
            return True
        else: 
    ...
    def compilationUnit(self):   
        ...
        while (self.tokenAtual.valor == "import" and self.analisaImport()):
            self.proximoToken()

        #CHECA SE EXISTE DECLARAÇÃO DE CLASSES
        while (self.tokenAtual.valor in modifiers and self.typeDeclaration()):
            self.proximoToken()
